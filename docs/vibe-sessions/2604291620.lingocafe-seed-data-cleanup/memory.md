...
- `42go.ngrok.app` is currently matched by the `default` app config, not `quicklist`; the default app only injects `/app-themes/_default/style.css`.
...
- QuickList theme overrides now live at `public/app-themes/quicklist/style.css`; app-theme injection only links registered `style.css` files, so missing app stylesheets do not trigger strict MIME failures.
