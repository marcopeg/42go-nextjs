...
---
# Add config.match.header

## Historical Summary
This archived record consolidates the task’s retained intent and decisions. Extend the AppConfig matching system to support HTTP header-based app resolution. This enables app selection based on custom headers, API keys, or request metadata.

## Retrieval Anchors
- `ACI`
- `docs/backlog/tasks/ACI-add-config-match-header-aci`
- `ACI.task.md`
- `AppConfigItem.match`
- `matchAppID`
- `config.match.url`
- `AppConfigItem.match.header`
- `src/AppConfig.ts`
- `Header match found for app: ${appKey}`

## Durable Outcome and Decisions
- The source preserves the task’s scope through these sections: Goals; Acceptance Criteria; Implementation Details; Development Plan; Next Steps.
...
- Consolidated artifacts: `ACI.task.md`.
