...
This task is only about saving the email, note and join newsletter checkbox in the database.
The "join newsletter" checkbox should be optional and should be a boolean that must be set to true in the configuration.
The visual feedback after insertion can be a markdown message passed as configuration, or a redirect url that we should navigate to after adding the email.
