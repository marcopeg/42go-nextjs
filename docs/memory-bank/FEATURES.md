...
**Features**: App-level fallback inheritance, dynamic database-backed metadata, credentialed private manifests, explicit installer-document identity boundaries, seamless client navigation outside installers, exact target-aware standalone UI, Chromium prompt integration, and Apple/Safari manual instructions through the shared Modal.
