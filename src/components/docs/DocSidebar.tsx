...
// Utility function to normalize doc URLs (removes README from paths)
function normalizeDocUrl(url: string): string {
  // Remove leading slash if present
  const cleanUrl = url.startsWith("/") ? url.slice(1) : url;

  // If the URL ends with /README or /README.md or /README.mdx, remove it
  const normalizedUrl = cleanUrl
    .replace(/\/README\.mdx?$/, "")
    .replace(/\/README$/, "");

  // Add back the leading slash for absolute URLs
...
export default function DocSidebar({
  ...
  const customComponents = {
      ...
      if (typeof props.href === "string") {
        ...
        if (
          ...
          if (props.href.startsWith("./")) {
            ...
            // Normalize URL to remove README parts
            transformedHref = normalizeDocUrl(transformedHref);
            ...
            transformedHref = `/${parentSegments.join(
              "/"
    ...
    h2(props: { children?: React.ReactNode; [key: string]: unknown }) {
      return (
        <h4 className="text-md font-semibold mb-1 mt-3">{props.children}</h4>
      );
    },
