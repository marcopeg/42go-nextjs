...
class DocCache {
  ...
  set(key: string, data: DocFile): void {
    ...
    const size =
      data.content.length * 2 + JSON.stringify(data.metadata).length * 2;
...
export function doesDocExistSync(slug: string): boolean {
  ...
  // Try README fallback
  const readmeMdPath = path.join(docsDirectory, normalizedSlug, "README.md");
  const readmeMdxPath = path.join(docsDirectory, normalizedSlug, "README.mdx");

  return existsSync(readmeMdPath) || existsSync(readmeMdxPath);
}
...
export async function doesDocExist(slug: string): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      // Try README fallback
      const readmeMdPath = path.join(
        docsDirectory,
        normalizedSlug,
        "README.md"
      );
      const readmeMdxPath = path.join(
        docsDirectory,
        normalizedSlug,
        "README.mdx"
      );
...
export async function getDocContent(slug: string): Promise<string | null> {
  ...
  try {
    ...
    try {
      ...
      // If direct file not found, try README fallback
      const readmeMdPath = path.join(
        docsDirectory,
        normalizedSlug,
        "README.md"
      ...
      try {
        return await fs.readFile(readmeMdPath, "utf8");
      } catch {
        try {
          return await fs.readFile(readmeMdxPath, "utf8");
        } catch {
          return null;
        }
      }
...
export async function getDoc(slug: string): Promise<DocFile | null> {
  ...
  try {
    ...
    try {
      ...
      try {
        content = await fs.readFile(readmeMdPath, "utf8");
        filePath = readmeMdPath;
        isMdx = false;
      } catch {
        try {
          content = await fs.readFile(readmeMdxPath, "utf8");
          filePath = readmeMdxPath;
          isMdx = true;
        } catch {
          return null;
        }
...
// Check if a URL path should be redirected (contains README)
export function shouldRedirectUrl(slugPath: string): string | null {
  // Check if the path ends with /README or contains /README/ or is just "README"
  if (
    slugPath === "README" ||
    slugPath.endsWith("/README") ||
    slugPath.includes("/README/")
  ) {
    // Remove /README from the path
    const cleanPath = slugPath
      .replace(/\/README(?:\/|$)/g, "/") // Remove /README/ or /README at end
      .replace(/^README(?:\/|$)/, "") // Remove README at start
      .replace(/\/$/, ""); // Remove trailing slash

    // If the clean path is empty, return empty string (will become /docs)
    return cleanPath || "";
  }
  return null;
}

// Normalize display labels by removing README parts
export function normalizeDocLabel(slug: string): string {
  return slug
    .replace(/\/README$/, "")
    .replace(/^README$/, "index")
    .replace(/^articles\/default\//, ""); // Also clean up default article paths
}
...
export async function getAllDocs(): Promise<string[]> {
  ...
  const docs: string[] = [];
  const readmeDirs = new Set<string>(); // Track directories with README files
  ...
  async function scanDirectory(dir: string, basePath: string = "") {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        // If this is a README file, add the directory path instead (without README)
        if (entry.name.startsWith("README.")) {
          const dirSlug = basePath || "index"; // Use "index" for root README
          if (!readmeDirs.has(dirSlug)) {
            docs.push(dirSlug);
            readmeDirs.add(dirSlug);
          }
        } else {
          docs.push(slug);
        }
