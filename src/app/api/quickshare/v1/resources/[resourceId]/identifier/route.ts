...
import { setQuickShareAutomationIdentifier } from '@/lib/quickshare/server/automation-service';
...
const setIdentifier = async (
  ...
  withQuickShareAutomationContext(request, async context => {
    ...
    const input = quickShareAutomationRequestSchemas.identifier.parse(
      await request.json().catch(() => undefined)
    );
    return quickShareApiJson({
      resource: await setQuickShareAutomationIdentifier(context.principal, resourceId, input),
    });
  });

export const POST = protectRoute(setIdentifier, quickShareAutomationPolicy);
